...
public class CoffeeMachine {
  ...
  private static Behavior<CoffeeMachineCommand> startBrewing(
    ...
    return Behaviors.withTimers(
        timers -> {
          timers.startSingleTimer(
              new CoffeeReadyTick(), Duration.ofSeconds(BREWING_DURATION_SECONDS));
  ...
  // Note: CoffeeReadyTick is an internal message scheduled by the actor's timer that signal that
  // brewing is completed.
  // We make it private as it is for internal implementation and shouldn't be exposed as external
  // protocol.
