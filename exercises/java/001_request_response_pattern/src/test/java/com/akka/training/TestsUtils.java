...
class TestsUtils {
    ...
    public static <T> CapturedLogEvent offsetCapturedLogEvent(BehaviorTestKit<T> testKit, int offsetFromEnd) {
        var logs = testKit.getAllLogEntries();
        if(offsetFromEnd > 0 && offsetFromEnd <= logs.size()) {
            return logs.get(logs.size()-offsetFromEnd-1);
        } else {
            throw new RuntimeException(
                String.format("Invalid offsetFromEnd parameter, %s is not within [0, logsSize]", offsetFromEnd));
        }
    }
