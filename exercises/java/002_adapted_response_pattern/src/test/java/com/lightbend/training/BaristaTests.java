...
public class BaristaTests {
    ...
    // Barista should log the work in progress orders when it receives a new order (OrderCoffee message)
    @Test
    public void baristaShouldLogReceivingOrder() {
        ...
        CapturedLogEvent expectedLogEvent = TestsUtils.expectedInfoLog(
    ...
    public void spawnCoffeeMachineChild() {
        ...
        var spawnEffectOpt = effects.stream().filter(e -> e instanceof Effect.Spawned)
            .findFirst();
    ...
    public void baristaShouldSendRequest() {
        ...
        BehaviorTestKit<Barista.BaristaCommand> testKit = BehaviorTestKit.create(Barista.create());
        TestInbox<CoffeeMachine.CoffeeMachineCommand> coffeeMachineInbox = testKit.childInbox("coffee-machine");
        ...
        // for reference see: https://doc.akka.io/docs/akka/current/typed/testing-sync.html#sending-messages
